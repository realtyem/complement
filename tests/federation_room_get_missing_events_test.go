...
func TestGetMissingEvents(t *testing.T) {
	...
	t.Run("parallel", func(t *testing.T) {
		t.Run("GapFilling", func(t *testing.T) {
			t.Parallel()
			// formerly TestGetMissingEventsGapFilling
			// users needed: @alice:hs1 and "fake" @bob
		...
		t.Run("OutboundFederationIgnoresMissingEventWithBadJSONForRoomVersion6", func(t *testing.T) {
			t.Parallel()
			// formerly TestOutboundFederationIgnoresMissingEventWithBadJSONForRoomVersion6
			// users needed: @alice:hs1 and "fake" @charlie
			alice := deployment.Client(t, "hs1", "@alice:hs1")
		...
		t.Run("InboundCanReturnMissingEvents", func(t *testing.T) {
			t.Parallel()
			// formerly TestInboundCanReturnMissingEvents
			// users needed: @alice:hs1 and "fake" charlie
			alice := deployment.Client(t, "hs1", "@alice:hs1")
			...
			for _, visibility := range []gomatrixserverlib.HistoryVisibility{
				...
				// do not t.Parallel() this inner sub-test as it will only actually test the last thing in the for
				// loop(plus a bunch of errors).
